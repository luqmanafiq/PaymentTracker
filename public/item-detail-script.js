document.addEventListener('DOMContentLoaded', function() {
    ...
    transactions.forEach((transaction) => {
        fetch(`/api/get-progress?transactionId=${transaction.dataset.transactionId}`)
            .then(response => response.json())
            .then(data => {
                const progressBar = document.getElementById(`progress-${transaction.dataset.transactionId}`);
